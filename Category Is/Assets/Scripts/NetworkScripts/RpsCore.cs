...
// the Photon server assigns a ActorNumber (LocalPlayer.ID) to each LocalPlayer, beginning at 1
// for this game, we don't mind the actual number
// this game uses LocalPlayer 0 and 1, so clients need to figure out their number somehow
public class RpsCore : MonoBehaviourPunCallbacks
{
    ...
    // public Hand randomHand;    // used to show remote LocalPlayer's "hand" while local LocalPlayer didn't select anything
	...
	// 	// disable the "reconnect panel" if PUN is IsConnected or connecting
	// 	if (PhotonNetwork.IsConnected && this.DisconnectedPanel.gameObject.GetActive())
	// 	{
	// 		this.DisconnectedPanel.gameObject.SetActive(false);
	// 	}
	// 	if (!PhotonNetwork.IsConnected && !PhotonNetwork.Connecting && !this.DisconnectedPanel.gameObject.GetActive())
	// 	{
    ...
    //     // show local LocalPlayer's selected hand
    ...
    //     // remote LocalPlayer's selection is only shown, when the turn is complete (finished by both)
    ...
    //     else
    //     {
	// 		ButtonCanvasGroup.interactable = PhotonNetwork.CurrentRoom.PlayerCount > 1;
    ...
    //         {
    //             // alpha of the remote hand is used as indicator if the remote LocalPlayer "is active" and "made a turn"
    ...
    // // when a LocalPlayer moved (but did not finish the turn)
    ...
    // // when a LocalPlayer made the last/final move in a turn
    ...
    //     {
    //         PhotonNetwork.LocalPlayer.AddScore(1);   // this is an extension method for Player. you can see it's implementation
    //     }
    // }
	...
	// 		this.TimeText.text = "";
    //         this.RemotePlayerText.text = "waiting for another LocalPlayer        00";
    //     }
    ...
    //         {
    //             // when the CurrentRoom has two players, start the first turn (later on, joining players won't trigger a turn)
    //             this.StartTurn();
    //         }
    //     }
    //     else
    //     {
    //         Debug.Log("Waiting for another LocalPlayer");
    //     }
    // }

    // public override void OnPhotonPlayerConnected(Player newPlayer)
    // {
    //     Debug.Log("Other LocalPlayer arrived");
    ...
    // public override void OnPhotonPlayerDisconnected(Player otherPlayer)
    // {
    //     Debug.Log("Other LocalPlayer disconnected! IsInactive: " + otherPlayer.IsInactive);
    // }
