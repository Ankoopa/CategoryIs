...
public class PlayerListingMenu : MonoBehaviourPunCallbacks
{
    ...
    void Update()
    {
        if (_reInstant)
        {
            photonView.RPC("ReInstatiatePlayer", RpcTarget.AllBufferedViaServer);
            _reInstant = false;
        }
    }
    ...
    [PunRPC]
    void _PlayerAvatar(int AvaIndex)
    {
        // _playersList.Avatar.sprite = ChoosingAvatar.ChooseAvatar().AvatarImage;
        Debug.Log(ChoosingAvatar.AvaIndex() + "is the Avatar");
        for (int i = 0; i < AvatarSprite.Length; i++)
        {
            if (i == AvaIndex)
            {
                _playersList.Avatar.sprite = AvatarSprite[i];
                
                Debug.Log("Changed Avatar Image" + AvatarSprite[i]);
            }
        }
        _reInstant = false;
    }
